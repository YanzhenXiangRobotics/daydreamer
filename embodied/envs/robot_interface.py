...
class UR5SimpleRobotWrapper:
    ...
    def get_robot_state(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        gripper_pos = self.get_gripper_pos() #? PickPlaceENv._get_tool_site
        servo_angle = self.robot.getj() # PickPlaceEnv._get_arm_dof_pos_normalized
        cart_pos = self.robot.getl() # PickPlaceEnv._get_ee_position
        return (
    ...
    def set_position(
        ...
        self.robot.movel(
            ...
            threshold=0.01,
            wait=wait,
        ) # PickPlaceEnv._step_impl

    def set_z(self, z: float, wait: bool = True) -> None:
        p = self.robot.get_pos()
        p.z = z
        self.robot.set_pos(p, acc=self.ACC, vel=self.VEL_Z, threshold=0.01, wait=wait) # PickPlaceEnv._step_impl
